...
public class Fetcher{
	 ...
	 */
	public Fetcher() {//NO NEED TO APPLY COMPRESSION HERE
		Compressor.clearDictionary();
		Text.clearText();
		try {
			File file = new File(DEFAULTPATH + "\\default.cmp");
	 ...
	 */
	public Fetcher(String line) {//APPLY COMPRESSION HERE FOR ONCE + RECOGNIZE DICTIONARY AND HANDLE IT
		Compressor.clearDictionary();
		Text.clearText();
		...
		while (isValidInput != true) {
			try {
				...
				String dictionaryLine = br.readLine();
				Compressor.updateDictionary(dictionaryLine);
				String sCurrentLine;
				while ((sCurrentLine = br.readLine()) != null) {
					Text.addToCompressedText(sCurrentLine);
				}
				isValidInput = true;
			} catch (IOException e) {
				String defaultName = DEFAULTPATH + "\\default.cmp";
